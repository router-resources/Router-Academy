    ...
    {Array.from({ length: repeatCount }, (_, i) => (
        <div key={i}>
          <div className="fruits-divst" onClick={async() => {
            ...
            <l style={{ color: 'rgb(185, 27, 100)' }} className={JSON.parse(localStorage.getItem("user")) && JSON.parse(localStorage.getItem("user"))[courseId].isModuleComplete[`${i}`]==true ||localStorage.getItem(`C${courseId}M${i}`)=='true' || JSON.parse(localStorage.getItem("userinfo")).basicprogramming.length>i && JSON.parse(localStorage.getItem("userinfo")).basicprogramming.includes(`s${i}`) ? 'fas fa-check-circle checked-icon' : 'far fa-circle unchecked-icon'}></l>
            <strong>
              &nbsp;
              Module &nbsp; {i+1}
              <br></br>
